...
if [ $(dpkg-query -W -f='${Status}' gdebi-core 2>/dev/null | grep -c "ok installed") -eq 1 ]; then
    echo "GDEBI-CORE: ✅ $(dpkg-query -W -f='${Version}' gdebi-core)"
else
...
if [ $(dpkg-query -W -f='${Status}' zoom 2>/dev/null | grep -c "ok installed") -eq 1 ]; then
    echo "ZOOM: ✅ $(dpkg-query -W -f='${Version}' zoom)"
else
...
if [ -x "$(command -v snap)" ]; then
    echo "SNAP: ✅ $(snap version | awk '/snapd/ {print $2}')"
else
...
if [ -x "$(command -v slack)" ]; then
    echo "SLACK: ✅  Na versão mais recente"
else
...
if [ $(dpkg-query -W -f='${Status}' code 2>/dev/null | grep -c "ok installed") -eq 1 ]; then
    echo "VSCODE: ✅ $(dpkg-query -W -f='${Version}' code)"
else
...
if [ $(dpkg-query -W -f='${Status}' google-chrome-stable 2>/dev/null | grep -c "ok installed") -eq 1 ]; then
    echo "GOOGLE CHROME: ✅ $(dpkg-query -W -f='${Version}' google-chrome-stable)"
else
...
if [ $(dpkg-query -W -f='${Status}' discord 2>/dev/null | grep -c "ok installed") -eq 1 ]; then
    echo "DISCORD: ✅ $(dpkg-query -W -f='${Version}' discord)"
else
...
if [ -x "$(command -v spotify)" ]; then
   echo "Spotify: ✅ $(dpkg -l spotify-client | grep spotify-client | awk '{print $3}')" 
else
